...
// Check the state of a server in a given directory.
// Returns null if no .ocmux.json exists, or an object with status.
function serverStatus(dir) {
  const sf = statefileFor(dir);
  const state = readState(sf);
  if (!state || !state.url) return null;
  const winIdx = windowByDir(dir);
  const tui = winIdx != null ? tuiPaneId(winIdx) : null;
  return {
    dir,
    url: state.url,
    status: winIdx != null ? 'alive' : 'dead',
    windowIndex: winIdx,
    hasTui: tui != null,
  };
}

// List dead servers by scanning known directories for orphaned .ocmux.json files.
// dirs: array of directories to check (e.g. from connections file).
// Returns entries with status 'dead' that are not already in listServers().
function listDeadServers(knownDirs) {
  const alive = new Set(listServers().map(s => s.dir));
  const dead = [];
  for (const dir of knownDirs) {
    if (alive.has(dir)) continue;
    const status = serverStatus(dir);
    if (status) dead.push(status);
  }
  return dead;
}
...
// Delegates to startServer for the actual server creation.
...
function resurrectServer(dir, printLogs) {
  ...
  if (state && state.window_index != null) {
    ...
    if (sessionR.status === 0) {
      ...
      if (actualName !== dir) {
        ...
        if (found == null) {
          // No window exists for this directory — old server is already dead, skip kill
          winIdx = null;
        } else {
          winIdx = found;
        }
      ...
      if (winIdx != null) {
        ...
        sleep(0.3);
        const killR = _tmux(['kill-window', '-t', `${SESSION}:${winIdx}`]);
        if (killR.status !== 0) {
          throw new Error(`Failed to kill old tmux window ${winIdx}: ${killR.stderr || 'unknown error'}`);
        }
        // Wait for window to actually disappear (up to 2s)
        for (let i = 0; i < 10; i++) {
          if (windowByDir(dir) == null) break;
          sleep(0.2);
        }
        if (windowByDir(dir) != null) {
          throw new Error(`Old tmux window for ${dir} still exists after kill`);
        }
      }
    }
  }
  ...
  // Start fresh server (same logic as ocmux serve)
  return startServer(dir, printLogs);
}

// Start a new opencode server in the given directory.
// Creates a tmux window, starts opencode serve, waits for the URL,
// and writes .ocmux.json. Returns { url, dir } on success.
function startServer(dir, printLogs) {
  // Normalize to real path so tmux window names are consistent regardless of symlinks
  try { dir = fs.realpathSync(dir); } catch {}
  ensureSession();
  const lf = logfileFor(dir);

  // Create a new tmux window and capture its index directly from the -P output
  ...
  _tmux(['send-keys', '-t', `${SESSION}:${winIdx}.0`,
    'opencode serve' + (serveFlags ? ' ' + serveFlags : '') + ` 2>&1 | tee '${lf}'`, 'Enter']);
  ...
  fs.writeFileSync(path.join(dir, '.ocmux.json'), JSON.stringify(newState) + '\n');
