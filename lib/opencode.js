...
function makeRequest(options, data, cancelRef) {
...
async function getSession(server, sessionId) {
  // Try dedicated endpoint first (newer opencode versions)
  try {
    const url = `${server}/session/${encodeURIComponent(sessionId)}`;
    const { status, body } = await makeRequest(buildJsonRequest(url, 'GET'));
    if (status === 200) {
...
function listenForSessionEvents(server, sessionId, callbacks, cancelRef) {
  process.stderr.write(`  [SSE] connecting to ${server}/event for session ${sessionId}\n`);
  return new Promise((resolve, reject) => {
    const url = `${server}/event`;
    const parsed = new URL(url);
    ...
    const req = http.request({
      ...
      process.stderr.write(`  [SSE] connected for session ${sessionId}\n`);
      ...
      const safetyTimer = setTimeout(() => {
        if (resolved) return;
        resolved = true;
        process.stderr.write(`  [SSE] safety timeout (90s) for session ${sessionId}, collected ${collected.length} chars\n`);
        cleanup();
        req.destroy();
        res.destroy();
        resolve(collected);
        ...
        while (buffer.includes('\n\n')) {
          ...
          try {
            ...
            if (event.type === 'session.status') {
              process.stderr.write(`  [SSE] session.status: type=${props.status?.type} status=${JSON.stringify(props.status)}\n`);
            } else {
              process.stderr.write(`  [SSE] event type=${event.type} sessionID=${props.sessionID} waitSession=${sessionId}\n`);
            }
            ...
            if (event.type === 'permission.asked') {
              ...
              process.stderr.write(`  [SSE] PERMISSION ASKED: ${permStr}\n`);
              if (callbacks.onPermission) {
                process.stderr.write(`  [SSE] calling onPermission callback\n`);
                callbacks.onPermission(props);
              } else {
                process.stderr.write(`  [SSE] WARNING: no onPermission callback registered\n`);
              }
              continue;
            }

            if (event.type === 'permission.replied') {
              process.stderr.write(`  [SSE] PERMISSION REPLIED: type=${props.type}\n`);
      ...
      res.on('error', (err) => {
        ...
        resolved = true;
        cleanup();
        process.stderr.write(`  [SSE] response stream error for session ${sessionId}: ${err.message}\n`);
